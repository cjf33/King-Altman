...
for i = 1:length(model.kinetic)
    ...
    if ~isempty(model.kinetic(i).act)
        for j = 1:length(model.kinetic(i).act)
            act_param{kk2} = strcat('k',num2str(k));
            act_param{kk2+1} = strcat('k',num2str(k+1),'*',activator{s2});
            participant{l} = activator{s2};
            k = k + 2;
            kk = kk + 2;
            s2 = s2 + 1;
            l = l + 1;
    ...
    if exist('act_param','var') == 1
        MMparam(i).act_param = act_param';
    else
        MMparam(i).act_param = [];
    end
